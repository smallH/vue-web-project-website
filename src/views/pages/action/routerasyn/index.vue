<template>
	<div id="routerasyn">
		<div class="bground">
			...
			<div class="line"></div>
			<div class="content">
				<div>懒加载也叫延迟加载，即在需要的时候进行加载，随用随载。在单页应用中，如果没有应用懒加载，运用webpack打包后的文件将会异常的大，造成进入首页时，需要加载的内容过多，延时过长，不利于用户体验，而运用懒加载则可以将页面进行划分，需要的时候加载页面，可以有效的分担首页所承担的加载压力，减少首页加载用时，仅在build打包代码时才用到懒加载。懒加载模块位于src/router：</div>
				<div class="code-title">懒加载配置：</div>
				<div class="md">
...
module.exports = (fileUrl) => {
	return resolve => require(['../views/' + fileUrl], resolve);
				...
				<div class="code-title">路由定义：</div>
				<div class="md">
					<pre v-highlightjs><code class="javascript">// router/index.js
import Vue from 'vue'
